...
export default class Search {
    ...
    sendRequest(){
        axios.post('/search', {_csrf: this._csrf, searchTerm: this.inputField.value}).then((response) => {
            ...
            // renderResultsHTML(response.data)
        }).catch(() => {
    ...
    renderResultsHTML(posts){
        if(posts.length){
            ...
            this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Soory, we could not find any result for that search</p>`
        }   
        this.hideLoaderIcon()
